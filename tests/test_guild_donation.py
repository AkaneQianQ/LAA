...
class TestPipelineValidation:
    ...
    def test_first_confirm_has_disappear_verification_chain(self):
        ...
        assert "wait_before_handle_first_confirm" in pipeline
        assert "handle_first_confirm" in pipeline
        assert "move_first_confirm_mouse_safe_position" in pipeline
        ...
        wait_before = pipeline["wait_before_handle_first_confirm"]
        assert wait_before.get("action", {}).get("param", {}).get("duration_ms") == 500
        assert wait_before.get("next") == ["handle_first_confirm"]

        first = pipeline["handle_first_confirm"]
        assert first.get("next") == ["wait_after_handle_first_confirm_click"]

        move_safe = pipeline["move_first_confirm_mouse_safe_position"]
        assert move_safe.get("action", {}).get("type") == "Move"
        assert move_safe.get("action", {}).get("param", {}).get("target") == [535, 339]
        assert move_safe.get("next") == ["verify_first_confirm_disappear"]

        wait_after_click = pipeline["wait_after_handle_first_confirm_click"]
        assert wait_after_click.get("next") == ["move_first_confirm_mouse_safe_position"]
