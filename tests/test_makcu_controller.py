...
class DummySerial:
    ...
    def write(self, data):
        ...
        elif stripped in {"km.left()", "km.right()", "km.middle()", "km.side1()", "km.side2()"}:
            response = f"{stripped}\r\n0\r\n>>> ".encode("utf-8")
        else:
            response = f"{stripped}\r\n>>> ".encode("utf-8")
...
def test_makcu_controller_logs_hardware_ack(serial_stub, caplog):
    ...
    assert any("MAKCU send" in message and "km.press('enter')" in message for message in messages)
    assert any("MAKCU ack" in message and "km.press('enter')" in message for message in messages)
...
def test_service_version_is_1_0_03():
    assert service_main.VERSION == "1.0.03"
