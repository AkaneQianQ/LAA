...
def download_and_apply_release(
    ...
    error_log_path = download_dir / "apply_update.error.log"
    ...
    script_path.write_text(
        "\n".join(
            [
                ...
                "    }",
                "    $restartCandidates = @(",
                f'        (Join-Path $InstallDir "{RELEASE_PRODUCT_NAME}.exe"),',
                "        $RestartFile",
                "    )",
                "    $restartFileName = [System.IO.Path]::GetFileName($RestartFile)",
                "    if ($restartFileName) {",
                "        $restartCandidates += (Join-Path $InstallDir $restartFileName)",
                "    }",
                "    $restartTarget = $restartCandidates | Where-Object { $_ -and (Test-Path -LiteralPath $_) } | Select-Object -First 1",
                "    if (-not $restartTarget) {",
                '        throw "restart executable not found after update"',
                "    }",
                "    Start-Process -FilePath $restartTarget -ArgumentList $restartArgs -WorkingDirectory $InstallDir",
                "    if ($CleanupDir) {",
                "        $cleanupCommand = @(",
                "            'Start-Sleep -Seconds 3'",
                "            \"if (Test-Path -LiteralPath ''$CleanupDir'') { Remove-Item -LiteralPath ''$CleanupDir'' -Recurse -Force -ErrorAction SilentlyContinue }\"",
                "        ) -join '; '",
                "        Start-Process -WindowStyle Hidden -FilePath powershell -ArgumentList @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', $cleanupCommand) -WorkingDirectory $InstallDir | Out-Null",
                "    }",
                "} catch {",
                ...
                "    }",
                "    if ($ErrorLogPath) {",
                '        Add-Content -LiteralPath $ErrorLogPath -Value ("[ERROR] " + $_.Exception.Message)',
                "    }",
                "    throw",
                "} finally {",
